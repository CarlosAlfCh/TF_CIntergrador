...
public class ClienteDAO implements CRUD<Cliente>{
    ...
    public int insertar(Cliente nuevo) {
        int r=0;
        String sql = "insert into persona (nombres,apelpat,apelmat,dni,correo,telefono,contrasena,estado,idrol,direccion,distrito) values (?,?,?,?,?,?,?,?,?,?,?)";
        try {
            conn = cn.conectar();
            ps = conn.prepareStatement(sql);
            ...
            ps.setString(11, nuevo.getDistrito());         
            r=ps.executeUpdate();
        } catch (Exception e) {
            System.out.println(e.toString());
